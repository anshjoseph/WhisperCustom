...
if __name__ == "__main__":
    ...
    if len(hotwords) <= 0:
        hotwords = None
    server = TranscriptionServer(use_vad=True,denoise=False,hotwords=hotwords,model_list=ASRs,no_speech_prob=0.45)
...
if __name__ == "__main__":
    ...
    if len(hotwords) <= 0:
        hotwords = None
    server = TranscriptionServer(use_vad=True,denoise=True,hotwords=hotwords,model_list=ASRs,no_speech_prob=0.45)
    parser = argparse.ArgumentParser()
    parser.add_argument('--port', '-p',
                        type=int,
                        default=9090,
